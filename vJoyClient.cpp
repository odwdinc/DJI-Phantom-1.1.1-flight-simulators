...
PyObject *pName, *pModule, *pDict, *pFuncS, *pFuncG;
...
void setupPython(std::string ComPort) {

	
	PyObject *pArgs, *pValue, *comPort;

	Py_Initialize();


	comPort = PyString_FromString(ComPort.c_str());


	pName = PyString_FromString("DJI_Interface");
	pModule = PyImport_Import(pName);
	Py_DECREF(pName);

	if (pModule != NULL) {
		pFuncS = PyObject_GetAttrString(pModule, "Setup");
		pFuncG = PyObject_GetAttrString(pModule, "getData");
		/* pFunc is a new reference */

		if (pFuncS && PyCallable_Check(pFuncS)) {
			pArgs = PyTuple_New(1);
			PyTuple_SetItem(pArgs, 0, comPort);
			pValue = PyObject_CallObject(pFuncS, pArgs);
			Py_DECREF(pArgs);
			if (pValue != NULL) {
...
std::string  GetControilsPython() {
	PyObject *pArgs, *pValue;
	std::string dataBack = "disconected";
	if (pFuncG && PyCallable_Check(pFuncG)) {
		pArgs = PyTuple_New(0);
		pValue = PyObject_CallObject(pFuncG, pArgs);
		Py_DECREF(pArgs);
		
		if (pValue && pValue != NULL) {
			std::string str(pValue->ob_type->tp_name);

			if (str.compare("NoneType") != 0) {
				dataBack = PyString_AsString(pValue);
				//printf("Result of call: %s\n", PyString_AsString(pValue));
			}
		}
		else {
...
_tmain(int argc, _TCHAR* argv[])
{
	...
	{
		wprintf(L"Vendor: %s\nProduct :%s\nVersion Number:%s\n", VENDOR_STR_ID, PRODUCT_STR_ID, static_cast<TCHAR *>(GetvJoySerialNumberString()));
	};
	...
	while (1)
	{
		...
		//std::cout << "ok :" << message.length() << '\n';
		try {
			message = GetControilsPython();
			if (message.compare("disconected") == 0) {
				RelinquishVJD(DevID);
				stopPython();
				setupPython(port);
				AcquireVJD(DevID);
				...
				if (message.length() > 0) {
					...
					iReport.wAxisZ = std::stol(getSval());
					iReport.wAxisX = std::stol(getSval());
					iReport.wAxisY = std::stol(getSval());
					iReport.wAxisXRot = std::stol(getSval());

					iReport.wAxisYRot = std::stol(getSval());
					iReport.wAxisZRot = std::stol(getSval());
